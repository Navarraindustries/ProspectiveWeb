...
def new_volume_id() -> str:
    """Identidad de un _volume.npy concreto; se regenera cada vez que se reescribe."""
    return uuid.uuid4().hex


def _cache_key(meta: dict, npy_path: Path) -> str:
    """Clave de la caché del navegador: identifica el VOLUMEN, no la sesión.

    Reanudar copia _volume.npy y su meta bajo un id de sesión nuevo; como el
...
def ensure_volume_cached(session_id: str) -> dict:
    ...
    orientation_manual, volume_id}. cache_key and orientation_manual are
    per-call, never persisted; volume_id is persisted with the cache and is
    regenerated by every path that rewrites _volume.npy.
